...
import com.myoa.dao.WFE.WFEFlowProcessMapper;
...
import org.springframework.transaction.annotation.Transactional;
 ...
 public class WFEFlowProcess extends WFE
 {
   ...
   public FlowProcess find(int id)
   {
     ...
     FlowProcess flowProcess = this.wfeFlowProcessMapper.find(maps);
     return flowProcess;
   }
 
   public List<FlowProcess> findProcseePrcsTo(Map<String, Object> map) {
     return this.wfeFlowProcessMapper.findProcseePrcsTo(map);
   }
   ...
   public ToJson<FlowProcess> updateByPrimaryKeySelective(FlowProcess record)
   {
     ...
     try {
       ...
       this.wfeFlowProcessMapper.updateByPrimaryKeySelective(record);
       tojson.setFlag(0);
       tojson.setMsg("OK");
     } catch (Exception e) {
       e.printStackTrace();
   ...
   {
     List list = this.wfeFlowProcessMapper.findFlowId(flowId);
     return list;
   }
   ...
   public ToJson<FlowProcess> delete(int id)
   {
     ToJson toJson = new ToJson();
     try {
       this.wfeFlowProcessMapper.deleteByPrimaryKey(Integer.valueOf(id));
       toJson.setFlag(0);
       toJson.setMsg("ok");
     } catch (Exception e) {
       toJson.setFlag(1);
   ...
   public ToJson<FlowProcessList> flowView(Integer flowId, Integer runId, String userId)
   {
     ...
     try {
       ...
       List list = this.wfeFlowProcessMapper.findF(flowId.intValue());
       List runPrcsList = new ArrayList();
       if (runId != null) {
         runPrcsList = this.wfeFlowRunPrcsMapper.selectAllByRunId(runId);
       }
       int len = list.size();
   ...
   public ToJson<FlowProcess> insertSelective(FlowProcess record) {
     ToJson toJson = new ToJson();
     try {
       this.wfeFlowProcessMapper.insert(record);
       toJson.setFlag(0);
       toJson.setMsg("ok");
   ...
   public FlowProcess findbyprcsId(int flowId, int prcsId) {
     ...
     FlowProcess flowProcess = this.wfeFlowProcessMapper.findbyprcsId(maps);
     return flowProcess;
   }
   ...
   {
     ToJson toJson = new ToJson();
     FlowProcess prcsTo = this.wfeFlowProcessMapper.findLine(flowProcess.getId());
     try {
       if ((prcsTo.getPrcsTo().equals("0")) || (prcsTo.getPrcsTo().equals("0,")))
         this.wfeFlowProcessMapper.updateTopAndLeft(flowProcess);
   ...
   {
     ToJson toJson = new ToJson();
     FlowProcess flowProcess = this.wfeFlowProcessMapper.findPrcessLine(flowProcesse.getFlowId(), flowProcesse.getPrcsId());
     FlowProcess fpr = new FlowProcess();
     try {
       if (StringUtils.checkNull(flowProcess.getPrcsTo()).booleanValue()) {
         fpr.setId(flowProcess.getId());
         fpr.setPrcsTo("0");
         this.wfeFlowProcessMapper.updateTopAndLeft(fpr);
       } else {
   ...
   public ToJson myFlowView(Integer flowId, Integer runId, String userId)
   {
     ...
     try {
       ...
       List list = this.wfeFlowProcessMapper.findF(flowId.intValue());
       List runPrcsList = this.wfeFlowRunPrcsMapper.selectMyAll(userId, runId);
       ...
       for (int i = 0; i < runPrcsListLenth; i++) {
         ...
         if (((DateFormat.getTime(((FlowRunPrcs)runPrcsList.get(i)).getPrcsTime()).intValue() != -621701856) && (((FlowRunPrcs)runPrcsList.get(i)).getPrcsId().intValue() > 1)) || ((DateFormat.getTime(((FlowRunPrcs)runPrcsList.get(i)).getPrcsTime()).intValue() == -621701856) && (((FlowRunPrcs)runPrcsList.get(i)).getPrcsId().intValue() > 1)))
         {
           ...
           String upTime = this.wfeFlowRunPrcsMapper.findTimeBy(params);
   ...
   public ToJson selectFlowFeedBackByRunId(String runId) {
     List datas = this.flowRunFeedbackMapper.selectFlowFeedBackByRunId(runId);
   ...
   public BaseWrappers getFlowRunPrecsAll(String runId) {
     ...
     List<FlowRunPrcs> datas = this.wfeFlowRunPrcsMapper.getFlowRunReacsAllByRunId(runId);
     Date newDate = new Date();
