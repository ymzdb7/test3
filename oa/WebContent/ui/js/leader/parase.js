...
var VKParase = {
    ...
    load: function() {
        ...
        $(document).on('click', '#vk_parase_item tbody tr', function() {
            ...
            if (tag === 'textarea') {
                var ckeEle = $('#cke_' + VKParase.$tn.attr("id"));
                if(ckeEle.length>0){ //判断是不是ckeditor
                    var value = eval("CKEDITOR.instances."+VKParase.$tn.attr("id")+".getData()");
                    value = value + $(this).attr('item_name');
                    CKEDITOR.instances[VKParase.$tn.attr("id")].setData(value);
                }else{
