...
(function($){
   ...
   $(document).ready(function(){
      ...
      $("#sub_module_org_0 li").live('mouseenter', mouseenter_org0_node_callback);
      $("#sub_module_org_0 li").live('mouseleave', mouseleave_org0_node_callback);
      $("#sub_module_org_0 li span[op]").live('mouseleave', mouseleave_org0_node_op_callback);

      $("#sub_module_org_1 li").live('mouseenter', mouseenter_org0_node_callback);
      $("#sub_module_org_1 li").live('mouseleave', mouseleave_org0_node_callback);
...
function xmlDom2TreeNodes(dom, recursive, flag)
{
   ...
   var nodes = [];
   var node, node_name, node_id, node_title, is_org, user_id, dept_id, sex, url, uid_index, client;
...
function LoadRecent(flag)
{
   ...
   var recent_array = get_im_users(recent_uid);
   var html_tr = '';
   for(var i=0; i < recent_array.length; i++)
   {
      var user = recent_array[i];
      if(!user)
         continue;
      var uid = user.uid;
      var uid_index = jQuery.inArray(uid, online_uid_array);
...
function mouseenter_org0_node_callback()
{
   var node = jQuery(this).attr("dtnode");
