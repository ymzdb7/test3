...
<div class="bx">
	<div class="center">
		<div class="navigation  clearfix" style="margin-top: -66px;">
			<div class="left" >
				<img src="/img/commonTheme/${sessionScope.InterfaceModel}/gzjhcx.png"/>
				<div class="news">
			...
			<form id="queryWorkPlanForm">
				<div class="middle">
					...
					<div class="le publisher">
						...
						<textarea name="toPersonName" id="toPersonName" class="toPersonName" user_id="" cols="24"></textarea>
					...
					<div class="le publisher">
						...
						<textarea name="custodion1" id="participatorName" class="participatorName" user_id="" cols="24"></textarea>
					...
					<div class="le publisher">
						<div class="color" style="width:175px;text-align: center"><fmt:message  code="event.th.PersonCharge"/>：</div>
						<textarea name="managerName" id="managerName" class="managerName" user_id="" cols="24"></textarea>
			...
			<div>
				<table id="tr_td">
					...
					<tr>
						...
						<td class="th" ><fmt:message  code="event.th.PlanName"/></td>
						<td class="th titleOne"><fmt:message  code="sup.th.startTime"/></td>
						<td class="th"><fmt:message  code="meet.th.EndTime"/></td>
						<td class="th"><fmt:message  code="event.th.PlanCategory"/></td>
						<td class="th"><fmt:message  code="event.th.PersonCharge"/></td>
						<td class="th"><fmt:message  code="event.th.Participanta"/></td>
						<%--<td class="th"><fmt:message  code="email.th.file"/></td>--%>
						<td class="th"><fmt:message  code="notice.th.state"/></td>
						<td class="th notice_do" style="width:20%"><fmt:message  code="notice.th.operation"/></td>
					</tr>
					</thead>
					<tbody id="j_tb"></tbody>
				</table>
...
<script>
    ...
    function queryWorkPlanByCond(output) {
        if(output!=1){//查询按钮
			...
			ajaxPage.data.toPersonId=$("#toPersonName").attr("user_id"),
            ajaxPage.data.participator=$("#participatorName").attr("user_id"),
            ajaxPage.data.manager=$("#managerName").attr("user_id"),
            ...
            window.location.href='<%=basePath%>/workPlan/selectWorkPlanByCond?output='+output+'&name='+$("#name").val()+
                ...
                '&type='+$("#type").val()+'&toId='+$("#toName").attr("deptid")+'&toPerson='+$("#toPersonName").attr("user_id")
				+'&participator='+$("#participatorName").attr("user_id")+'&manager='+$("#managerName").attr("user_id")+'&remark='+$("#remark").val();
        }
    }
...
//                                '<td><input type="checkbox"></td>' +
                           ...
                           /* if(data[i].attachmentId=="" || data[i].attachmentId=="undefined"){
                            ...
                            str+= '<td><a class="edit_btn btn" href="../../workPlan/workPlanEdit?editFlag=1&planId='+data[i].planId+'"><fmt:message  code="depatement.th.modify"/></a>' +
                                ' &nbsp;' +
                            ...
                            if(data[i].opinionLeader==$.cookie("uid")+","){
                                str+='<span class="approve_btn btn" planId="'+data[i].planId+'" style="cursor: pointer"><fmt:message code="workflow.th.pizhu" /></span>' +
                                    ' &nbsp;';
                            }
                            if(data[i].status==2){
                                if(data[i].suspendFlag==0){
                                    str+= '<span class="goOn_btn btn" planId="'+data[i].planId+'" onclick="modifyStatus(1,'+data[i].planId+')" style="cursor: pointer"><fmt:message  code="event.th.Continue"/></span>'+ ' &nbsp;';
                                }else{
                                    str+='<span class="suspend_btn btn" planId="'+data[i].planId+'" onclick="modifyStatus(0,'+data[i].planId+')" style="cursor: pointer"><fmt:message  code="event.th.suspend"/></span>'+ ' &nbsp;';
                                }
                                str+='<span class="end_btn btn" planId="'+data[i].planId+'" onclick="modifyStatus(3,'+data[i].planId+')" style="cursor: pointer"><fmt:message  code="meet.th.End"/></span></td>';
                            }else{
                                str+='<span class="effec_btn btn" planId="'+data[i].planId+'" onclick="modifyStatus(2,'+data[i].planId+','+data[i].status+')" status="'+data[i].status+'" style="cursor: pointer"><fmt:message  code="notice.state.effective"/></span></td>';
                            }
                            str+='</td></tr>';
                        }
                ...
                for(var i=0;i<data.length;i++){
                  ...
                  /*  if(data[i].attachmentId==""||data[i].attachmentId=="undefined"){
                    ...
                    str+='<td>'+data[i].writeTime+'</td>' +
                        '<td><span class="edit_btn" id="editDetail" onclick="editDetail('+data[i].detailId+',1)"><fmt:message code="depatement.th.modify" /></span>&nbsp;' +
