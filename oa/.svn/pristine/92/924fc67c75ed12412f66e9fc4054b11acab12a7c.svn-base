...
<head>
    ...
    <title><fmt:message code="attend.th.OutwardAndApproval" /></title>
...
<script>
    ...
    var ajaxPage={
        ...
        page:function (num) {
         ...
         $.get('/attendOut/backlogAttendList',me.data,function (json) {
                var str='';
                var arr=json.datas
                for(var i=0;i<arr.length;i++){
                    str+='<tr>' +
                        '<td>'+arr[i].prcsName+'</td>' +
                        '<td>'+arr[i].deptName+'</td>' +
                        '<td>'+arr[i].outTime1+'</td>' +
                        '<td>'+arr[i].outTime2+'</td>' +
                        '<td>'+arr[i].outType+'</td>' +
                        ...
                        if(me.data.num==1){
                            return '<a href="/workflow/work/workform?opflag=1&flowId='+arr[i].flowId+'&prcsId='+arr[i].realPrcsId+'&flowStep='+arr[i].step
                                +'&runId='+arr[i].runId+'&tabId='+arr[i].outId+'&tableName='+arr[i].tableName+'&isNomalType=true" target="_blank" style="margin-right: 10px"><fmt:message code="document.th.handle" /></a>' +
                                '<a href="/workflow/work/workformPreView?flowId='+arr[i].flowId+'&flowStep='+arr[i].step+'&tabId='+arr[i].outId+'&tableName='+arr[i].tableName+'&runId='+arr[i].runId+'&prcsId='+arr[i].realPrcsId+'" target="_blank"><fmt:message code="roleAuthorization.th.ViewDetails" /></a>'
                        }else {
                            return '<a href="/workflow/work/workformPreView?flowId='+arr[i].flowId+'&flowStep='+arr[i].step+'&tabId='+arr[i].outId+'&tableName='+arr[i].tableName+'&runId='+arr[i].runId+'&prcsId='+arr[i].realPrcsId+'" target="_blank"><fmt:message code="roleAuthorization.th.ViewDetails" /></a>'
                        }
                            }()+
                        '</td>' +
                        '</tr>'
