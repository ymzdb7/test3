   ...
   void encode(byte[] in, int inPos, int inAvail, BaseNCodec.Context context) {
     if (!context.eof)
       if (inAvail < 0) {
         ...
         byte[] i = ensureBufferSize(this.encodeSize, context);
         int buffer = context.pos;
         switch (context.modulus) {
         case 0:
           break;
         case 1:
           i[(context.pos++)] = this.encodeTable[(context.ibitWorkArea >> 2 & 0x3F)];
           i[(context.pos++)] = this.encodeTable[(context.ibitWorkArea << 4 & 0x3F)];
           if (this.encodeTable != STANDARD_ENCODE_TABLE) break;
           i[(context.pos++)] = 61;
           i[(context.pos++)] = 61; break;
         case 2:
           i[(context.pos++)] = this.encodeTable[(context.ibitWorkArea >> 10 & 0x3F)];
           i[(context.pos++)] = this.encodeTable[(context.ibitWorkArea >> 4 & 0x3F)];
           i[(context.pos++)] = this.encodeTable[(context.ibitWorkArea << 2 & 0x3F)];
           if (this.encodeTable != STANDARD_ENCODE_TABLE) break;
           i[(context.pos++)] = 61; break;
         ...
         for (int var8 = 0; var8 < inAvail; var8++) {
           byte[] var9 = ensureBufferSize(this.encodeSize, context);
           ...
           if (0 == context.modulus) {
             var9[(context.pos++)] = this.encodeTable[(context.ibitWorkArea >> 18 & 0x3F)];
             var9[(context.pos++)] = this.encodeTable[(context.ibitWorkArea >> 12 & 0x3F)];
             var9[(context.pos++)] = this.encodeTable[(context.ibitWorkArea >> 6 & 0x3F)];
             var9[(context.pos++)] = this.encodeTable[(context.ibitWorkArea & 0x3F)];
   ...
   void decode(byte[] in, int inPos, int inAvail, BaseNCodec.Context context)
   {
     if (!context.eof) {
       ...
       for (int buffer = 0; buffer < inAvail; buffer++) {
         byte[] buffer1 = ensureBufferSize(this.decodeSize, context);
         byte b = in[(inPos++)];
         if (b == 61) {
           context.eof = true;
           break;
         ...
         if ((b >= 0) && (b < DECODE_TABLE.length)) {
           ...
           if (result >= 0) {
             ...
             if (context.modulus == 0) {
               buffer1[(context.pos++)] = (byte)(context.ibitWorkArea >> 16 & 0xFF);
               buffer1[(context.pos++)] = (byte)(context.ibitWorkArea >> 8 & 0xFF);
               buffer1[(context.pos++)] = (byte)(context.ibitWorkArea & 0xFF);
             }
           }
         }
       }
       ...
       if ((context.eof) && (context.modulus != 0)) {
         byte[] var9 = ensureBufferSize(this.decodeSize, context);
         switch (context.modulus) {
         case 1:
           break;
         case 2:
           context.ibitWorkArea >>= 4;
           var9[(context.pos++)] = (byte)(context.ibitWorkArea & 0xFF);
           break;
         case 3:
           context.ibitWorkArea >>= 2;
           var9[(context.pos++)] = (byte)(context.ibitWorkArea >> 8 & 0xFF);
           var9[(context.pos++)] = (byte)(context.ibitWorkArea & 0xFF);
           break;
         default:
           throw new IllegalStateException("Impossible modulus " + context.modulus);
