...
<div class="head w clearfix">
    <ul class="index_head">
        <li data-num="0"><span class="headli1_1" id="waitwork"><fmt:message code="workflow.th.wait" /></span><img class="headli1_2" src="../../img/twoth.png" alt="">
        </li>

        <li data-num="1"><span class="headli2_1 endWork one"  id="endWork"><fmt:message code="workflow.th.gowork" /></span><img src="../../img/twoth.png" alt="" class="headli2_2">
        </li>
...
<div class="body" style="width: 100%;margin-top: 44px">
    <div class="bx">
        <div class="navigation  clearfix">
            <div class="left">
                <img src="/img/commonTheme/${sessionScope.InterfaceModel}/banjiegongzuo.png">
                <div class="news"><fmt:message code="workflow.th.gowork" />
                </div>
                <input id="flow_search_value_bj" class="inp" type="text" placeholder="&nbsp;<fmt:message code="workflow.th.Searchforname" />">
                <div id="btn_search" class="search"><h1 style="cursor:pointer;" class="subsearch" id="bjgz_search"><fmt:message code="workflow.th.sousuo" /></h1></div>
            </div>

            <div class="button-operation">
                <button type="button" class="btn btn-success" onclick="clicked5();"><img src="../../img/mywork/newbuildworjk.png" style="margin-right: 3px;margin-bottom: 1px;"><fmt:message code="main.newwork" /></button>
                ...
                <button type="button" class="btn btn-info" id="refresh1" onclick="clicked2();"><img src="../../img/mywork/shuxinwork.png" style="margin-right: 8px;margin-bottom: 1px;"><fmt:message code="global.lang.refresh" /></button>
            </div>
...
<script type="text/javascript">
    ...
    function bzlct(e){
        eventas=e;
        window.open('/flowSetting/processDesignToolTwo?flowId='+e.attr('flowid')+'&rilwId='+e.attr('runid'),'');
    }
    var pageObj=$.tablePage('#pagediv','1458px',[
        {
            width:'75px',
        ...
        },
        {
            ...
            selectFun:function (workLevel,obj) {
                var workLeverl = getWorkLevle(obj.flowRun.workLevel);
                if(obj.state == '执行中'){
                    var color = "red";
                }else{
                    var color = "green";
                ...
                return '<a class="wenhao" style="cursor: pointer;" href="javascript:void(0);" onclick ="js_method($(this))" title='+obj.flowRun.runName.split(' ')[0]+' runId='+obj.runId+' flowId='+obj.flowRun.flowId+' prcsId='+obj.flowProcess.prcsId+' flowstep='+obj.prcsId+'><span class="font-green" style="color:'+ fontcolor +'">【'+workLeverl+'】</span>'+obj.flowRun.runName+'</a><br><span>&nbsp;<fmt:message code="workflow.th.type"/>:</span><span class='+color+'>'+obj.state+'</span>';
            }
        },
        {
            width:'96px',
            ...
            width:'187px',
            title:'<fmt:message code="workflow.th.Stepstake"/>',
            name:'prcsId',
            selectFun:function (prcsId,obj) {
//                var status = handleData(obj.prcsFlag);
                return '<a class="wenhao" style="cursor: pointer;" onclick="bzlct($(this))" runid="'+ obj.flowRun.runId +'" flowid="'+ obj.flowRun.flowId +'"><span><fmt:message code="workflow.th.First" />'+prcsId+'<fmt:message code="workflow.th.step" />:'+obj.flowProcess.prcsName+'</span><br><span><fmt:message code="notice.th.state" />:'+ obj.state1 +'</span></a>';
            }
        },
        {
            width:'201px',
            ...
            name:'prcsTime',
            selectFun:function (prcsTime,obj) {
                var thiss = '';
                if(prcsTime == ""){
                    thiss = '未接收';
                }else{
                ...
                if(obj.deliverTime == ""){
                    ...
                    thiss2 = '办结于:' + obj.deliverTime;
                }
                return '<fmt:message code="workflow.th.ArriveTo" />:'+obj.createTime+'<br>'+thiss+'<br>'+thiss2;
            }
        },
        {
            width:'157px',
        ...
        },
        {
            ...
            selectFun:function (arriveTime,obj) {
                if(obj.state == '已结束'){
                    return '<fmt:message code="meet.th.IsOver" />'
                }else{
                    if(obj.state2 == 1){
                        return '<a class="wenhao" style="cursor: pointer;" onclick="bzlct($(this))" runid="'+ obj.flowRun.runId +'" flowid="'+ obj.flowRun.flowId +'"><fmt:message code="attend.th.CurrentStep" />:<fmt:message code="adding.th.Concurrent" /><br><fmt:message code="doc.th.CurrentManager" />:<fmt:message code="adding.th.Concurrent" /><br><fmt:message code="ending.th.CurrentState" />:'+obj.state+'</a>';
                    }else{
                        return '<a class="wenhao" style="cursor: pointer;" onclick="bzlct($(this))" runid="'+ obj.flowRun.runId +'" flowid="'+ obj.flowRun.flowId +'"><fmt:message code="attend.th.CurrentStep" />:'+ obj.bz +'<br><fmt:message code="doc.th.CurrentManager" />:'+ obj.currentPeople +'<br><fmt:message code="ending.th.CurrentState" />:'+obj.state+'</a>';
                    }
                }
            }
        },
        ...
        me.init('../../workflow/work/selectEndWord',[{
    ...
    $(document).on('click','.editAndDelete0',function () {
        var obj=pageObj.arrs[$(this).attr('data-i')]
        window.open('/workflow/work/workformPreView?flowId='+obj.flowRun.flowId+'&prcsId='+obj.flowProcess.prcsId+'&flowStep='+obj.prcsId
            +'&runId='+obj.runId);
    })
    ...
    $('#bjgz_search').click(function () {
        pageObj.data.myworkconditions=$("#flow_search_value_bj").val();
        pageObj.init();
    })
</script>
