...
<script type="text/javascript">
                    ...
                    success:function(reg){
                        ...
                        for(var i=0;i<data.length;i++){
                            str+='<tr class="pagediv" id='+data[i].voteId+' data-have="'+data[i].have+'"> ' +
                                ...
                                '<td>' +
                                '<a href="javascript:;" class="look" sid="'+data[i].id+'" sname="'+data[i].schoolName+'" style="color:#1772c0">转出</a> ' +
                                '</td>' +
                                '</tr>'
    ...
    $(document).on('click','.look',function(){
        ...
        layer.open({
            type: 1,
            title:['学籍转出', 'background-color:#2b7fe0;color:#fff;'],
            area: ['500px', '300px'],
            shadeClose: true, //点击遮罩关闭
            // btn: ['创建', '取消'],
            content:'<form id="saveRule" class="layui-form"><div class="inputlayout">' +
            '<ul>' +
            '<li class="clearfix" style="padding: 30px 60px;">' +
            '<label>原学校 ：</label><input id="deptNos" type="text" disabled="disabled" name="portalsNo" value="'+sname+'"><b style="padding-left: 10px;"></b>' +
            '</li>' +
            '<li class="clearfix" style="padding-left: 52px;">' +
            '<label>转入学校：</label><input  class="td_title1 test_null" id="dept" name="schoolCode" type="text" readonly/>'+
            '<a href="javascript:;" id="chose" style="font-weight: normal;margin-left: 10px">选择</a>' +
            '</li>' +
