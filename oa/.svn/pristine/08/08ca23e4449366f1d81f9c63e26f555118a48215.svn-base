...
<script>
            ...
            <%--$.get('/document/selMyDocSendOrReceive',me.data,function (json) {--%>
                ...
                <%--for(var i=0;i<arr.length;i++){--%>
                    <%--str+='<tr>' +--%>
                        ...
                        <%--function () {--%>
                            <%--if(arr[i].step==1){--%>
                                <%--return '<a href="javascript:void(0)"  data-id="'+arr[i].runId+'" class="delete_b"><fmt:message code="global.lang.delete"/></a>'+'<a href="/workflow/work/workformPreView?flowId='+arr[i].flowId+'&tabId='+arr[i].id+'&tableName='+arr[i].tableName+'&flowStep='+arr[i].step+'&runId='+arr[i].runId+'&prcsId='+arr[i].realPrcsId+'&isNomalType=false&hidden=true" target="_blank"><fmt:message code="roleAuthorization.th.ViewDetails"/></a>'--%>
                            <%--}else {--%>
    ...
    <%--$('.Query').click(function () {--%>
        ...
        <%--ajaxPage.page()--%>
    <%--})--%>
    <%--$('.pagediv').delegate('.delete_b','click',function () {--%>
