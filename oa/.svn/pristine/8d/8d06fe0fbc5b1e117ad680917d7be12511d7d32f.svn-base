                    ...
                    success:function(rsp){
                        ...
                        if(data2.attachmentName!=''){ //判断是否存在附件
                            for(var i=0;i<arr.length;i++){
                                <%--stra+='<span><a href="<%=basePath %>download?'+arr[i].attUrl+'" style="text-decoration:none;cursor: pointer;display:block;"><img style="margin-right:10px;" src="../img/attachment_icon.png"/>'+arr[i].attachName+'</a></span>';--%>
                                stra+='<div class="dech" deUrl="'+arr[i].attUrl+'"><a href="<%=basePath %>download?'+arr[i].attUrl+'" NAME="'+arr[i].attachName+'*" style="text-decoration:none;"><img style="margin-right:10px;" src="../img/attachment_icon.png"/>'+arr[i].attachName+'</a><img class="deImgs" style="margin-left:5px;cursor: pointer;" src="../img/file/icon_deletecha_03.png"/><input type="hidden" class="inHidden" value="'+arr[i].aid+'@'+arr[i].ym+'_'+arr[i].attachId+',"></div>';
                            }
                        }
