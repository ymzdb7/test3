...
<head>
    ...
    <title><fmt:message code="attend.th.BusinessApproval" /></title>
...
<script>
    ...
    var ajaxPage={
        ...
        page:function (num) {
         ...
         $.get('/attendEvection/backlogAttendList',me.data,function (json) {
                var str='';
                var arr=json.datas
                    ...
                    str+='<tr>' +
                        ...
                        if(me.data.num==1){
                            return '<a href="/workflow/work/workform?opflag=1&flowId='+arr[i].flowId+'&prcsId='+arr[i].realPrcsId+'&flowStep='+arr[i].step
                                +'&runId='+arr[i].runId+'&tabId='+arr[i].evectionId+'&tableName='+arr[i].tableName+'&isNomalType=true" target="_blank" style="margin-right: 10px"><fmt:message code="document.th.handle" /></a>' +
                                '<a href="/workflow/work/workformPreView?flowId='+arr[i].flowId+'&flowStep='+arr[i].step+'&tabId='+arr[i].evectionId+'&tableName='+arr[i].tableName+'&runId='+arr[i].runId+'&prcsId='+arr[i].realPrcsId+'" target="_blank"><fmt:message code="roleAuthorization.th.ViewDetails" /></a>'
                        }else {
                            return '<a href="/workflow/work/workformPreView?flowId='+arr[i].flowId+'&flowStep='+arr[i].step+'&tabId='+arr[i].evectionId+'&tableName='+arr[i].tableName+'&runId='+arr[i].runId+'&prcsId='+arr[i].realPrcsId+'" target="_blank"><fmt:message code="roleAuthorization.th.ViewDetails" /></a>'
                        }
                            }()+
                        '</td>' +
                        '</tr>'
